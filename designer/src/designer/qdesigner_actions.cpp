...
bool QDesignerActions::openForm(QWidget *parent)
{
    ...
    const QStringList fileNames = QFileDialog::getOpenFileNames(parent, tr("Open Form"),
        m_openDirectory, tr("Flexview UI files (*.%1);;All Files (*)").arg(extension), 0, QFileDialog::DontUseSheet);
...
bool QDesignerActions::saveFormAs(QDesignerFormWindowInterface *fw)
{
    ...
    const  QString saveFile = getSaveFileNameWithExtension(fw, tr("Save Form As"), dir, tr("Flexview UI files (*.%1);;All Files (*)").arg(extension), extension);
    if (saveFile.isEmpty())
        return false;
        ...
        if (workbench()->openForm(fn, &errorMessage)) {
            ...
            if (box.clickedButton() == updateButton) {
                ...
                fn = QFileDialog::getOpenFileName(core()->topLevel(),
                                                  ...
                                                  tr("Flexview UI files (*.%1);;All Files (*)").arg(extension), 0, QFileDialog::DontUseSheet);
...
{
    DockedMainWindow::flex->widgetList.clear();
